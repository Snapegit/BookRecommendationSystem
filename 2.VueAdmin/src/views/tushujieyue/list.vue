	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.jieyuefangshi" 
								placeholder="借阅方式"
								>
								<el-option v-for="item in jieyuefangshiLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
					<div class="search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('tushujieyue','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('tushujieyue','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('tushujieyue','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('tushujieyue','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('tushujieyue','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('tushujieyue','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.jieyuefangshi&&searchQuery.value.jieyuefangshi!=''){
			params['jieyuefangshi'] = searchQuery.value.jieyuefangshi
		}
	...
	//搜索
	const jieyuefangshiLists = ref([])
	const getjieyuefangshiLists = () => {
		jieyuefangshiLists.value = '自取,送上门'.split(',')
	}
	getjieyuefangshiLists()
	const searchClick = () => {
	...
	const approvalClick = (row) => {
		let params = {
			...
			jieyuefangshi: row.jieyuefangshi,
