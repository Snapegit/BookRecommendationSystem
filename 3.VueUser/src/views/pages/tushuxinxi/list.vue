				...
				<div class="data_view">
					<div class="data_item animation_box" v-for="(item,index) in list" :key="index" @click.stop="detailClick(item.id)" >
						...
						<div class="data_content">
							...
							<div class="data_title">{{item.tushuleixing}}</div>
...
<script setup>
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.tushuleixing = categoryList.value[categoryIndex.value]
		}
	...
	const detailClick = (id) => {
		router.push('tushuxinxiDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
